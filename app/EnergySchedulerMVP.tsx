...
function pillCls(active: boolean): string {
  return active
    ? "rounded-2xl border border-[#D4946A] bg-[#E8A87C] px-4 py-3 text-left text-[#3D1A08] min-h-[52px] w-full"
          ...
          return (
            <div key={block.id} className="absolute left-10 right-2 rounded-lg bg-[#E8D0B8] border border-[#D4946A] px-2 flex items-center overflow-hidden"
              ...
              <div className="min-w-0">
                <div className="text-xs font-medium text-[#3D1A08] truncate">{block.title}</div>
  ...
  return (
    <>
      ...
      <button onClick={onSave} disabled={!!eventFormError || !eventForm.title.trim()}
        className="w-full min-h-[48px] rounded-2xl bg-[#E8D0B8] px-4 text-base font-medium text-[#3D1A08] hover:bg-[#D4946A] disabled:opacity-40 disabled:cursor-not-allowed transition">
        Add event
      </button>
    </>
  );
...
export default function DayPlannerDecidesForYou() {
  ...
  function EventRow({ event }: { event: FixedEvent }) {
    return (
      <div className="flex items-center gap-3 rounded-2xl border border-[#E8D0B8] bg-[#E8D0B8] px-4 py-3 min-h-[56px]">
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium text-[#3D1A08] truncate">{event.title}</div>
          <div className="text-xs text-[#7A4A2A] mt-0.5">{formatTime(event.startHour, event.startMinute)} – {formatTime(event.endHour, event.endMinute)}</div>
        </div>
  ...
  const nowCard = plan.currentEvent ? (
    <div className="rounded-3xl border border-[#D4946A] bg-[#E8D0B8] px-5 py-4 shadow-sm">
      ...
      <div className="text-sm text-[#7A4A2A] mt-0.5">
    ...
    return (
      <div className="min-h-screen bg-[#FDF6EE] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-[#C8A87C]">Loading…</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        <div className="mx-auto max-w-2xl px-4 pt-5">
          ...
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs text-[#C8A87C]">
              {dbLoading ? "Syncing…" : `Signed in as ${user.email}`}
            </span>
            <button
              onClick={async () => { await supabase.auth.signOut(); router.push("/login"); }}
              className="flex items-center gap-1.5 rounded-xl bg-[#F5E6D3] px-3 py-1.5 text-xs text-[#7A4A2A] hover:bg-[#EDD8C0] min-h-[36px]"
            >
              ...
              <div className={cardCls}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-[#7A4A2A] mt-0.5">{plan.segment.label} · creativity {plan.segment.creativity}</div>
                  </div>
