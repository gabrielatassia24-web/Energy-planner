      ...
      <div className="w-full max-w-sm">
        ...
        {step === "email" ? (
          ...
          <form onSubmit={handleVerifyCode} className="space-y-4">
            <div className="rounded-3xl border border-[#F5CF82] bg-[#FFFBF0] p-4 text-center mb-2">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-[#7A6050]">Verification code</label>
              <input type="text" inputMode="numeric" pattern="[0-9]*" maxLength={6}
                value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))}
                placeholder="123456" autoComplete="one-time-code"
                className="w-full rounded-2xl border border-[#F5CF82] bg-white px-4 py-3 text-center text-2xl tracking-widest text-[#3D2B1F] outline-none transition focus:border-[#82A8F5]"
                required />
            </div>
            {error && <p className="rounded-xl bg-red-50 px-4 py-2 text-sm text-red-600">{error}</p>}
